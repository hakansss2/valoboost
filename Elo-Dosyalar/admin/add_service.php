...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            // Hizmeti ekle
            $stmt = $conn->prepare("
                INSERT INTO services (game_id, name, description, price, status, created_at)
                VALUES (?, ?, ?, ?, ?, NOW())
            ");
            $stmt->execute([$game_id, $name, $description, $price, $status]);
            
            $_SESSION['success'] = "Hizmet başarıyla eklendi.";
            header("Location: services.php");
            exit;
        } catch(PDOException $e) {
            $errors[] = "Hizmet eklenirken bir hata oluştu.";
        }
    }
}
?>
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="h3 mb-0 text-gray-800">Yeni Hizmet</h1>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    watchFormChanges('addServiceForm');
});
</script>
